...
// ContainerUIEntry is one container row for a file manager (friendly folder name + real CID string).
...
// ListContainersForUI returns sorted folder names and a map displayName → container ID string.
// Logic matches the FUSE / Explorer root listing (friendly name from ContainerGet, collision fallback to CID).
